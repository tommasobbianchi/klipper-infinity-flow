...
install() {
    ...
    echo "  Next steps:"
    echo ""
    echo "  1. Get your FlowQ refresh token:"
    echo "       python3 $(realpath "$SCRIPT_DIR")/flowq_setup_token.py"
    echo ""
    echo "  2. Add to moonraker.conf:"
    echo "       [infinity_flow]"
    echo "       refresh_token: YOUR_TOKEN_HERE"
    echo ""
    echo "  3. Add to printer.cfg:"
    echo "       [infinity_flow]"
    echo "       extruder: extruder"
    echo "       pause_mode: all_empty"
    echo "       swap_grace_period: 30"
    echo "       runout_gcode:"
    echo "           M117 Filament exhausted"
    echo "           PAUSE"
    echo ""
